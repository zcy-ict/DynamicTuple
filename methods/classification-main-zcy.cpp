...
extern int reduce_prefix_type;

extern bool dt_print_info;
...
void PerformClassificationZcy(CommandStruct &command, ProgramState *program_state, 
    ...
    for (int i = 0; i < traces_num; ++i) {
        ...
        if (command.method_name == "DynamicTuple_Demo")
            priority = (classifier.*LookupAccess)(traces[i], 0, rules[i], program_state);
        else
            priority = (classifier.*LookupAccess)(traces[i], 0, NULL, program_state);
...
int ClassificationMainZcy(CommandStruct command, ProgramState *program_state, vector<Rule*> &rules, 
    ...
    if (command.method_name == "DimTSS") {
        ...
        PerformClassificationZcy(command, program_state, multilayertuple, rules, traces, ans, &Classifier::Lookup, &Classifier::LookupAccess);
    } else if (command.method_name == "DynamicTuple_Demo") {
        DynamicTuple dynamictuple;
        dynamictuple.method_name = command.method_name;
        dynamictuple.use_port_hash_table = 0;
        dt_print_info = true;
#ifndef TUPLEINFO
        printf("DynamicTuple_Demo need to define TUPLEINFO\n");
        exit(1);
#endif
        vector<Rule*> rules2= GenerateOneMatchRules(rules);
        sort(rules2.begin(), rules2.end(), CmpRulePriority);
        vector<Trace*> traces2 = GenerateTraces(rules2);
        command.force_test = 1;
        vector<int> ans2 = GenerateAns(rules2, traces2, command.force_test);
        PerformClassificationZcy(command, program_state, dynamictuple, rules2, traces2, ans2, &Classifier::Lookup, &Classifier::LookupAccess);
        FreeTraces(traces2);
    } else if (command.method_name == "DynamicTuple_Basic") {
        DynamicTuple dynamictuple;
        dynamictuple.method_name = command.method_name;
        dynamictuple.use_port_hash_table = 0;
        PerformClassificationZcy(command, program_state, dynamictuple, rules, traces, ans, &Classifier::Lookup, &Classifier::LookupAccess);
    } else if (command.method_name == "DynamicTuple") {
        DynamicTuple dynamictuple;
        dynamictuple.method_name = command.method_name;
        dynamictuple.use_port_hash_table = 1;
        dynamictuple.threshold = 7;
        PerformClassificationZcy(command, program_state, dynamictuple, rules, traces, ans, &Classifier::Lookup, &Classifier::LookupAccess);
    } else if (command.method_name == "DynamicTuple_Dims") {
        MultilayerTuple multilayertuple;
        reduce_prefix_type = DYNAMICTUPLEDIMS_TYPE;
        prefix_dims_num = command.prefix_dims_num;
        create_next_layer_rules_num  = 1e9;
        delete_next_layer_rules_num = create_next_layer_rules_num / 2;
        multilayertuple.Init(1, true);
        PerformClassificationZcy(command, program_state, multilayertuple, rules, traces, ans, &Classifier::Lookup, &Classifier::LookupAccess);
    } else {
